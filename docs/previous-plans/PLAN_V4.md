# Question Pipeline -- Complete Plan v4 (V3 edited)
## Overview
...
│   │
│   └── vocabulary.ts                  # Controlled vocabulary: 100+ topic + tag mappings
│
├── data/
...
interface QuestionFile {
  ...
  year: number | null;        // null for ncert-exemplar (uses class instead)
...
interface Question {
  ...
  section: string;             // Free-form: "a" | "b" | "section-1" | "section-2" | null
  type: QuestionType;
  text: string;                // Question text (English)
  textHi: string | null;       // Hindi translation (NEET only, else null)
  options: string[] | null;    // null for assert-reason (auto-generated) and nat
  answer: string;              // MCQ: 0-based index as string ("0"-"3"). NAT: numeric value as string
  answers: string[] | null;    // Multiple correct for MSQ. Array of 0-based index strings
  ...
  difficulty: "easy" | "medium" | "hard" | null;  // null by default. Assigned by human reviewer. AI sets null.
  tags: string[];              // From controlled vocabulary (src/vocabulary.ts)
  revision: number;            // Bumped on each correction
  source: "official-pdf" | "reconstructed" | "imported-kaggle" | "imported-dataset";
  confidence: "high" | "medium" | "low" | null;  // null for official-pdf, set for reconstructed/imported
}

type QuestionType = "mcq" | "msq" | "nat" | "assertion-reason";
...
interface Passage {
  ...
  diagrams: Diagram[] | null;  // Diagrams referenced in the passage (NEET graphs/tables)
...
- **paper.json**: Full paper as-is. All subjects interleaved as in the original exam. number = 1-N where N = exam total questions (90 for JEE Main, 200 for NEET, 45 for JEE Adv per paper).
- **physics.json / chemistry.json / mathematics.json / biology.json**: Subject-split. number = 1-N where N = that subject's total for the shift. NOT hardcoded to 30. JEE Adv physics may be 18, NEET physics 50.
- **all-{subject}.json** at exam root: ALL questions of one subject across all shifts/years. number resets to 1-N sequentially across the entire aggregate. IDs remain unique per question. Original per-shift number can be derived from the ID.
...
13. passageId references a valid Passage if passageId !== null (any type can reference a passage).
...
const topicAliases: Record<string, string> = {
  ...
  // ... 100+ mappings defined in src/vocabulary.ts
}
```
...
const typeRules: Record<QuestionType, ValidationRule[]> = {
  ...
  "passage": []  // type is NOT "passage". Validated by passageId !== null for any type
};

### Difficulty Rubric (manual assignment during review)

| Level | Criteria |
|---|---|
| easy | Single concept, direct formula application, no tricks. 60%+ students expected to answer correctly |
| medium | 2+ concepts, minor calculation or reasoning step. 30-60% expected correct |
| hard | Multi-step, conceptual trick, or unusual application. <30% expected correct |

AI extraction sets `difficulty: null` for all questions. The human reviewer assigns difficulty during verification. This prevents the AI from guessing incorrectly.
```
...
| Matrix Match / Match Columns | NO | YES | NO | NO | Stored as `type: mcq` with 4 pairing options |
| Passage-Based Comprehension | YES | YES | YES | NO | passageId on any Question type references a Passage object |
...
Mistral OCR returns per-page images as base64-encoded strings. Diagram extraction uses a two-tier approach:

**Tier 1 (preferred): Mistral individual image extraction**
- Mistral's `image_pages` endpoint may return extracted individual images if diagrams are clearly separated from text.
- When available: directly decode each base64 image and save as PNG.
- Set file path, label (from alt text or figure caption), and caption (from surrounding text).

**Tier 2 (fallback): Full-page crop**
- If Mistral returns only full-page images, crop the diagram region using bounding box coordinates.
- The structurer.ts prompt asks AI to identify diagram regions: `"For each figure, note: page number and approximate location."`
- AI returns crop coordinates: `{ page: 3, x: 100, y: 200, width: 400, height: 350 }`.
- Sharp or Jimp library crops the region from the page image.
- If coordinates are unreliable: store full-page images and reference with page number + diagram index.
...
| Module | What to test |
|---|---|
| auto-validator.ts | Each of 30+ checks on known-good and known-bad inputs. Mock questions with intentional errors |
| field-checker.ts | Type rules fire correctly per question type. Edge cases: null options, empty arrays |
| id-assigner.ts | ID generation is deterministic and collision-free. Stability across re-runs |
| normalizer.ts | LaTeX to Unicode mapping. Whitespace normalization. OCR error corrections |
| topic-normalizer.ts | Alias resolution. Unknown topic fallback |
| hash-utils.ts | SHA-256 consistency. Empty input, large input |
| integrity.ts | Checksum compute/verify on known files. Mismatch detection |

### Integration Tests

| Scenario | What to test |
|---|---|
| End-to-end single question | Download test PDF -> OCR -> structure -> validate -> finalize. Output matches expected JSON |
| Diagram extraction | Mock Mistral response with base64 images. Verify PNG files written to correct path |
| Review CLI session | Mock user input sequence. Verify .review-progress.json is written correctly |
| Batch process | Mock all external APIs. Verify data/index.json updated correctly |

### Snapshot Tests

- Run structurer.ts on a known PDF, save the extracted JSON as a snapshot.
- On subsequent runs, diff against the snapshot.
- Fails intentionally when extraction changes, prompting human review of the diff.

### Mocking External APIs

- All AI-dependent modules accept injectable API clients.
- Tests use `nock` or `fetch-mock` to return controlled responses.
- Mistral OCR mock: return known markdown + base64 images.
- Cerebras/Gemini mock: return known JSON from test fixtures.
...
- src/vocabulary.ts: 100+ controlled topic + tag mappings.
...
### v4 Additional Flaws (14 more, caught in re-review)

| # | Flaw | v4 Fix |
|---|---|---|
| 29 | `type: "passage"` is not a real type. Passage-based questions are mcq/msq/nat with passageId | Removed `"passage"` from QuestionType. Validator checks `passageId !== null` for any type |
| 30 | `type: "match"` unnecessary. Match-columns are just MCQ with 4 pairing options | Removed `"match"` from types. Stored as `type: mcq`. User confirmed this is how JEE Adv works |
| 31 | `section: "a" \| "b"` too tight. JEE Adv has 3+ sections per subject | Changed to `section: string` (free-form). SectionConfig defines actual structure |
| 32 | Passage object had no diagram support. NEET passages often have graphs/tables | Added `diagrams: Diagram[]` to Passage interface |
| 33 | NCERT has no year, only class. Schema required year: number | Changed `year: number | null` across QuestionFile. null for ncert-exemplar |
| 34 | Diagram cropping from page images underspecified. No fallback | Added two-tier extraction: Tier 1 (Mistral individual images) + Tier 2 (coordinate-based crop from full-page) |
| 35 | Tags have no controlled vocabulary. Only topic-vocabulary existed | Renamed to `vocabulary.ts` with both topic AND tag mappings |
| 36 | No test strategy anywhere in the plan | Added Section 8.5: vitest unit tests, integration tests, snapshot tests, mocking strategy |
| 37 | MCQ `answer: "2"` ambiguous (index vs literal). Not documented | Added explicit comment: "MCQ: 0-based index as string ('0'-'3'). NAT: numeric value as string" |
| 38 | Subject counts hardcoded as 1-30. JEE Adv physics is 18, NEET physics is 50 | Changed Section 4.3: "number = 1-N where N = that subject's total per shift. Not hardcoded" |
| 39 | Aggregate file numbering unspecified. all-physics.json number field undefined | Documented: "number resets to 1-N sequentially across aggregate. IDs remain unique" |
| 40 | Difficulty assigned by AI with no rubric. Arbitrary and unreliable | AI sets difficulty = null. Human assigns during review using defined rubric (easy/medium/hard) |
| 41 | No confidence field for reconstructed/imported questions | Added `confidence: "high" | "medium" | "low" | null` optional field |
| 42 | Checksum approach correct but wording unclear | Already correct: "SHA-256 of JSON before adding checksum field". Wording improved for clarity |

### Total: 42 flaws identified and fixed across v1 -> v4.
...
### Match the Columns (JEE Advanced — stored as MCQ, 4 pairing options)
